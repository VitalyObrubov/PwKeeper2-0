...
#define  PIN_BTN_UP       9      // Button UP
#define  PIN_BTN_DN       15     // Button DOWN
#define  PIN_BTN_LEFT     16      // Button LEFT
#define  PIN_BTN_RIGHT    11     // Button RIGHT
#define  PIN_BTN_OK       10      // Button OK
...
#define  BTN_LEFT          0x26    // Left
#define  BTN_RIGHT         0x27    // Right
#define  BTN_OK            0x28    // Ok
#define  BTN_MENU          0x29    // Menu == LongOk
...
{   
  public:
    ...
    OneButton mBtUp, mBtDn, mBtLeft, mBtRight, mBtOk;     // Button handlers
    ...
    CGlobalData() : mpfRun(0), mpfRunPrev(0), mpfDisp(0), mpfDispPrev(0),
                    ...
                    mBtLeft( PIN_BTN_LEFT, true), mBtRight( PIN_BTN_RIGHT, true),
                    mBtOk( PIN_BTN_OK, true)
...
const byte PinNoFast[] = { 4,5,6,7,8,18,19,20,21,0 };
...
void hBtOkLongSt()  
  { gD.mKeyDnLp = 1; gD.mKey = BTN_MENU; 
    }
 void hBtOkLongEnd() 
 { gD.mKeyDnLp = 0; 
 }
...
{ FlashRd();  // Read data from eeprom
  ...
  mBtLeft.setClickTicks( 300 );
  mBtRight.setClickTicks( 300 );
  mBtOk.setClickTicks( 300 );
  ...
  mBtLeft.attachClick( hBtLeftSingle );
  mBtRight.attachClick( hBtRightSingle );
  
  mBtOk.attachClick( hBtOkSingle );
  mBtOk.attachLongPressStart( hBtOkLongSt );
  mBtOk.attachLongPressStop( hBtOkLongEnd );
...
{ int   i, n;
  // Serial.print("fSelect gD.mKey ");
  // Serial.println(gD.mKey, HEX);
  switch( gD.mKey )
  {  case BTN_UP:
     ...
     case BTN_RIGHT:
        ...
        break;    
    //  case BTN_ESC: //Edit record
    //     gD.mChanged = 0;
    //     gD.mPosMenu = 0;            
    //     gD.SetFunc( gD.fRecMenu, gD.fDispRecMenu );
    //     break;
     case BTN_MENU:
     case BTN_LEFT: //Settings
        gD.mPosMenu = 0;
        memset( gD.mEditStr, 0, sizeof(gD.mEditStr) );
...
{ char  c;
  ...
  {  case BTN_UP:    c = 0x20; break; // UP pressed          
     ...
     case BTN_RIGHT: c = 0x40; break; // RIGHT pressed     
     case BTN_LEFT:  c = 0x50; break; // LEFT pressed     
     
     case BTN_MENU:  // LongOK pressed
        return -1;
     case BTN_OK:  // OK pressed
        return 1;
  }
  ...
  if( 0x40 == c ) mPosEdit++; // NextPos.
  if( 0x50 == c ) 
    {mPosEdit--;
     if (mPosEdit < 0 ) mPosEdit = 0;
    }   // PrevPos.

  if( 0x20 == c ) mEditStr[mPosEdit] = c | (mEditStr[mPosEdit] + 1); //Increase digit.
  if( 0x00 == c ) mEditStr[mPosEdit] = c | (mEditStr[mPosEdit] - 1); //Decrease digit.
  return 0;
}
